// The 5,070-star naked-eye backdrop, from the HYG database.
//
...
module.exports = ({ok, get, run, ROOT}) => {
  ...
  ok(SKYFIELD && SKYFIELD.count === source.count,
     `console has ${SKYFIELD && SKYFIELD.count} stars, skyfield.json says ${source.count}`);
  ...
  ok(Math.max(...mags) <= 6.0, `a star fainter than the magnitude limit got in: ${Math.max(...mags)}`);
