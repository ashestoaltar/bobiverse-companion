...
module.exports = ({ok, get, run, each, need}) => {
  ...
  each("posts in Bill's voice", all.filter(x => x.voice === 'bobnet'), p => {
  ...
  const target = need('a post pointing at a specific record',
                      withLinks.about.map(a => a.split('/')).find(([, id]) => id));
  if (target) {
  ...
  const held = need('a record held at book 1',
                    get('BOBS').find(b => run('attestedAt')(b) >= 4));
  if (held) {
    state.book = 1;
